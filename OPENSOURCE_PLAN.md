...
## 4. Phase 1——密钥上收 + 协调层（✅ 开发完成 2026-07-27，待服务器部署启用）

完成状态：1.1 赛狐主动限流 ✅、1.2 sellfox_client 双模式 ✅（direct 回归通过；proxy 待
服务器 mcapi 重启新代码后联调）、1.3 审计（网关自动+本地上报）✅、1.4 占坑 API ✅
（测试矩阵 12 项全过，mcapi 6dc4e58）、1.5 四个写入点接占坑 ✅（build/po/placement/
inqsend，MCAPI_KEY 未配时全部跳过=现状零影响）、1.7 企微中继 ✅（回调落表/发送代理/
拉取/补拉；fba-docs 侧拉取导入=§4.5-I.2 未做）。
**启用步骤**：服务器 mcapi 拉新代码重启（自建三表）→ 配 API_KEYS_JSON + QIWE_TOKEN/GUID
→ qiwe 后台回调地址确认指向服务器 → 运营端配 MCAPI_KEY（+删 SELLFOX_* 即自动切 proxy）。
